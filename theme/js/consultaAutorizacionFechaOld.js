...
function enviarFormulario(){
  ...
  var fechaFin= $("#fechaFin").val(); 
    ...
    $.ajax({
      async: true,
      type: "POST",
      url:'Remito/getAutorizacionOldByFecha', 
      cache: false,
      data: postData,
      dataType: 'json',        
    })
    ...
    .fail(function() {
      alert("Error 92. Consulte con el administrador de sistemas.")
    }); 
}
...
function drawRow(rowData,nombreTabla) {
  ...
  $.each(rowData, function( index, value ) {
    ...
    row.append($("<td><button data-toggle='modal' data-target='.consultarRem' type='button'  onclick='verAutorizacion("+id+")'  class='btn seleccionar btn-default '><span class='fa fa-list'></span></button></td></tr></tbody>")); 
...
function verAutorizacion(Id){
  var id_autorizacion = Id; 
    ...
    $.ajax({
      async: true,
      type: "POST",
      url:'Remito/getAutorizacionOById', 
      cache: false,
      data: postData,
      dataType: 'json',        
    })
