$(".select2").select2();
cargarSelectNombreUnidad();

$('#selectUnidad').on("select2:select", function(e) { 
   consultarUnidadNombre();
});

$('#deleteUnidad').click(function(){
	deleteUnidad();
});
...
function deleteUnidad(){
		    ...
		    var data = $('#tablaPrincipal tr:gt(0)').map(function() {
		        return {
		            id_destino:  $(this.cells[0]).text()
		        };
		    }).get();
		    var postData = {
		      'id_destino' : data[0].id_destino,
		    };

		    $.post('Unidad/deleteUnidad', postData, function(data){
		        if (data) { 
		            alert("exito");
		            resetearForm();
		        }else{
...
function consultarUnidadNombre()
{
  var nombre = $("#selectUnidad").val(); 
  var postData = {
      'nombre' : nombre
    };
    $.ajax({
...
function cargarSelectNombreUnidad()
{
  $(function () {
     $.ajax({
            async: true,
    ...
    .done(function(data) {
      ...
      var $element = $('#selectUnidad').select2();
      $element.empty();
      ...
      $("#selectUnidad").append(option2).trigger('change');  
  ...
  $.ajax({
      ...
      //data: {id_producto: "producto"},           
        ...
        .done(function(data) {
          ...
          $.getScript("/libreria/theme/js/eliminarUnidad.js");
        })
        .fail(function() {
          alert("error. Consulte con el administrador de sistemas.")
        });
